...
class JTNNDecoder(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size, embedding, device):
    ...
    def forward(self, mol_batch, x_tree_vecs):
        ...
        pred_contexts.append( torch.LongTensor(list(range(batch_size))).to(self.device) )
        ...
        for t in range(max_iter):
            ...
            #Clique embedding
            cur_x = torch.LongTensor(cur_x).to(self.device)
            ...
            cur_batch = torch.LongTensor(batch_list).to(self.device)
        ...
        stop_hidden = torch.cat([cur_x,cur_o], dim=1)
        stop_hiddens.append( stop_hidden )
        stop_contexts.append( torch.LongTensor(list(range(batch_size))).to(self.device) )
        ...
        pred_targets = torch.LongTensor(pred_targets).to(self.device)
    ...
    def decode(self, x_tree_vecs, prob_decode):
        ...
        stack = []
        init_hiddens = torch.zeros(1, self.hidden_size).to(self.device)
        zero_pad = torch.zeros(1,1,self.hidden_size).to(self.device)
        contexts = torch.LongTensor(1).zero_().to(self.device)
        ...
        for step in range(MAX_DECODE_LEN):
            ...
            cur_x = torch.LongTensor([node_x.wid]).to(self.device)
            cur_x = self.embedding(cur_x)
